...
- **Notatka i zdjęcie-dowód na każdym poziomie**: każdy wiersz drabinki rozwija się (chevron) w edytor — krótka notatka (maks. 500 znaków) i zdjęcie (np. screen z aplikacji biegowej; JPG/PNG/WEBP/GIF, maks. 4 MB). Wpis (`MilestoneEntry`) jest **niezależny od odhaczenia** — odznaczenie poziomu ani rekalkulacja automatu nie kasują notatki/zdjęcia. Zdjęcia zapisuje warstwa storage ([src/lib/uploads.ts](src/lib/uploads.ts)): lokalnie na dysk (`uploads/`, poza `public/`, gitignored), a w chmurze (gdy jest `BLOB_READ_WRITE_TOKEN`) do **Vercel Blob**. W obu przypadkach serwowane wyłącznie przez autoryzowane API `/api/milestone-photo/[id]` — każdy użytkownik widzi tylko swoje zdjęcia. **Data zaliczenia** poziomu wyświetla się na wierszu drabinki („✓ Zaliczono 3 lipca 2026"), w nagłówku aktywności i na karcie w spisie aktywności.
...
## Hosting (Vercel + Neon + Blob)

Aplikacja jest przystosowana do darmowego wdrożenia 24/7: **Vercel** (Next.js) + **Neon** (PostgreSQL) + **Vercel Blob** (zdjęcia). Baza działa na PostgreSQL (`provider = "postgresql"`, `DATABASE_URL` z pulą + `DIRECT_URL` do migracji), a warstwa zdjęć sama przełącza się na Blob w chmurze. Pełna instrukcja klik-po-kliku: **[DEPLOY.md](DEPLOY.md)**.

> Praca lokalna też korzysta z PostgreSQL (darmowa baza/gałąź w Neon) — patrz DEPLOY.md, „Krok 2".
